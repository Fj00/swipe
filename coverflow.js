...
(function( $ ) {
	...
	$.widget( "ui.coverflow", {
		...
		_create: function () {
			...
			if( o.trigger.enter ) {
				this._on( this.items, { mouseenter : this._select });
			}
		...
		_init : function () {
			...
			console.log(parseInt($('#topArticle').css('margin-left')))
			css.left = parseInt($('#topArticle').css('margin-left'))
			css.width = $('#coverflow img').width() * $('#coverflow img').length;//width of image container
			css.height = 100;//height
			console.log(parseInt($('#coverflow img').css('width')) * $('#coverflow img').length + 'px');
		...
		_getCenterPosition : function () {//adjusts container left / right while clicking/dragging
			var pos;
			pos = - this.currentIndex * this.itemSize / 2;
			pos += this.outerWidth / 2 - this.itemSize / 2;
			pos -= parseInt( this.element.css( "paddingLeft" ) ,10 ) || 0;
			pos = Math.round( pos );
			pos = parseInt($('#topArticle').css('margin-left'));
			return { left : pos };//adjust left position of image container
		},
