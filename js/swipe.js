...
(function ($) {
	...
	$(function(){
		...
		var szl = function(e){
			if (!b_isRunning){
				...
				if (b_duplicate){	// if article already in queue, don't add it
					...
					//console.log(i_szlCount);
					var $lastSzld = $queueItems.filter(':last');
		...
		var setZ = function(szldItem, $this){
			if (szldItem < 0) {
				$this.css({
		...
		var queueDrag = {
			...
			drag: function(e, ui){
				...
				//console.log($('.szld').eq(lastOne).offset());
			...
			stop: function(e, ui){
				...
				var timeDiff = ((endTime - dragStartTime)/1000),
				...
				var $lastSzld = $('#queue .szld').eq($('.szld').length - 1);
