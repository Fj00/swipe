...
var sStoryName = 13;
var sStoryHome = 14;
var sStoryPrediction = 15;
var sStoryForwardingUser = 16;
var sStoryForwardingUserId = 17;
var sStoryForwardingSource = 18;
var sStoryComments = 19;
...
//console.log( story_comments.length );
    ...
    var story_link_href = '<a href="' + story_link + '" target="_blank" >Read more</a>';
      ...
      //story_full = story_full.replace( /.\u0080/gi ,'' );
		...
		if ( story_vote == 0 ) {
			storyData.push( [ story_title, story_summary, story_full, story_id, story_title_long, story_long, story_img, story_vote, -1, last_story_loaded_index, story_link, story_date, story_source_id, story_name, story_home, story_prediction, story_forwarding_user, story_forwarding_user_id, story_forwarding_source, storyComments ] );
      storyHash[ story_id ] = {
...
function DisplayStory( currStory, forward ){
  ...
  tt += '  <ul id="navsub-1">';
  tt += '    <li id="always-li" style="border-top: 1px #888888 solid;" onClick="RateSource(' + sID + ',1,0);"><input type="checkbox" id="always"><span style="color: #8E0B2E; margin-left: 4px; margin-right: 4px;">Always receive articles from this source</span></li>';
  tt += '    <li id="sometimes-li" onClick="RateSource(' + sID + ',0,0);"><input type="checkbox" id="sometimes"><span style="color: #8E0B2E; margin-left: 4px; margin-right: 4px;">Sometimes receive articles from this source</span></li>';
  tt += '    <li id="never-li" onClick="RateSource(' + sID + ',-1,0);"><input type="checkbox" id="never"><span style="color: #8E0B2E; margin-left: 4px; margin-right: 4px;">Never receive articles from this source</span></li>';
  tt += '  </ul>';
  ...
  cc += '  <table style="background-color: #FFEEDD;">';
  cc += '    <tr>';
  cc += '      <td rowspan="3" style="border-top: solid 2px #FFEEDD; width: 46px;">';
  cc += '        <div id="szzzl-buttons-' + ( jj + 1 ) + '">';
  cc += '          <a href="#szzzl" onclick=RateComment(' + comment_id + ',1); return false;"><img src="http://www.szzzl.com/images/comment-szzzl.png" /></a>';
  cc += '          <a href="#fzzzl" onclick=RateComment(' + comment_id + ',-1); return false;"><img src="http://www.szzzl.com/images/comment-fzzzl.png" style="margin-top: 10px;" /></a>';
  cc += '        </div>';
  cc += '      </td>';
  cc += '      <td style="border-top: solid 2px #FFEEDD; border-right: solid 1px #888888; background-color: #FFFFFF; padding-left: 3px; padding-right: 3px;">';
  ...
  $(document).ready(function(){
    ...
    if ( source_ids.indexOf( currStory[sStorySourceId] ) > -1 ) {
      $('#always-li').css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
      $('#always-li').css('background-image','-o-linear-gradient(-90deg,  #FFFFFF 0%, #CCFFCC 100%)');
      $('#always-li').css('background-image','-ms-linear-gradient(-90deg,  #FFFFFF 0%, #CCFFCC 100%)');
      $('#always-li').css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
      $('#always-li').css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
      $('#always').attr('checked',true);
      $('#sometimes').attr('checked',false);
      $('#never').attr('checked',false);
    } else {
      $('#sometimes-li').css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
      $('#sometimes-li').css('background-image','-o-linear-gradient(-90deg,  #FFFFFF 0%, #FFFF99 100%)');
      $('#sometimes-li').css('background-image','-ms-linear-gradient(-90deg,  #FFFFFF 0%, #FFFF99 100%)');
      $('#sometimes-li').css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
      $('#sometimes-li').css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
  ...
  if ( rating == 1 ) {
    ...
    $(document).ready(function(){
      if ( cyclic == 0 ) {
        ...
        $('#source-' + id).css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
        $('#source-' + id).css('background-image','-o-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
        $('#source-' + id).css('background-image','-ms-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
        $('#source-' + id).css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
        $('#source-' + id).css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #CCFFCC 100%)');
    ...
    $(document).ready(function(){
      if ( cyclic == 0 ) {
        ...
        $('#source-' + id).css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
        $('#source-' + id).css('background-image','-o-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
        $('#source-' + id).css('background-image','-ms-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
        $('#source-' + id).css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
        $('#source-' + id).css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #FFFF99 100%)');
        $('#source-' + id).attr('onclick','RateSource(' + id + ', 1, 1)');
      }
    });
  } else if ( rating == -1 ) {
    ...
    $(document).ready(function(){
      if ( cyclic == 0 ) {
        ...
        $('#sometimes-li').css('background-image','');
        $('#sometimes-li').css('background-image','');
        $('#never-li').css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
        $('#never-li').css('background-image','-o-linear-gradient(-90deg,  #FFFFFF 0%, #FFCCCC 100%)');
        $('#never-li').css('background-image','-ms-linear-gradient(-90deg,  #FFFFFF 0%, #FFCCCC 100%)');
        $('#never-li').css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
        $('#never-li').css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
        $('#always').attr('checked',false);
        $('#sometimes').attr('checked',false);
        $('#never').attr('checked',false);
        $('#never').attr('checked',true);
      } else if ( cyclic == 1 ) {
        $('#source-' + id).css('background-image','linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
        $('#source-' + id).css('background-image','-o-linear-gradient(-90deg,  #FFFFFF 0%, #FFCCCC 100%)');
        $('#source-' + id).css('background-image','-ms-linear-gradient(-90deg,  #FFFFFF 0%, #FFCCCC 100%)');
        $('#source-' + id).css('background-image','-moz-linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
        $('#source-' + id).css('background-image','-webkit-linear-gradient(-90deg, #FFFFFF 0%, #FFCCCC 100%)');
